...
public class AdminManageCrownParticles {

	private ItemStack angryVillager, waterBubble, dragonBreath, endRod, totem, heart, portal, redstone, smoke_normal, spell_mob_ambient, suspend_depth, water_wake, flame;
	private Inventory inv;

	public AdminManageCrownParticles(int size) {
		...
		inv = Bukkit.createInventory(null, size, "§5§oManage Crown Particles Menu");
	}
		
	public void createManageCrownParticlesMenu(Player p) {
		
		angryVillager = new ItemStack(Material.PLAYER_HEAD, 1);
		ItemMeta avM = angryVillager.getItemMeta();
		avM.addItemFlags(ItemFlag.HIDE_ATTRIBUTES);
		avM.setDisplayName("§6§lAngry Villager Particles");
		angryVillager.setItemMeta(avM);
		inv.setItem(10, angryVillager);
